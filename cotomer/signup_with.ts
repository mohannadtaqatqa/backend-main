...
const apisignup = app.post('/signup/auth', async (req: Request, res: Response) => {
  // const { fname, lastname, phone, city, address, pass, email } = req.body;
  const { fname, pass, email } = req.body;
  ...
  try {
    ...
    const sql = 'INSERT INTO customer (customer_fname, email) VALUES (?, ?)';
    conn.query(sql, [fname, email], (err, results) => {
      if (err) {
        console.error(err);
